...
eval_dataset = AvaSpeechActivityDataset('../ava_dataset_prepare/ava_activities.csv',
                                        '../ava_speech_dataset_one_channel', config['n_mels'])
eval_dataloader = DataLoader(eval_dataset, batch_size=1, collate_fn=collate_fn, shuffle=False)


print(evaluate(model, eval_dataloader, 1))
print(evaluate(model, eval_dataloader, 0.99))
print(evaluate(model, eval_dataloader, 0.97))
with open('metrics_avaspeech_gru_add.csv', 'w') as f:
    writer = csv.writer(f)
    for i, th in enumerate([0.4062, 0.625, 0.95, 0.97, 0.99]):
